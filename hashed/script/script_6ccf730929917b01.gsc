...
function private function_f08e8278(data, trace, eye_dir) {
    ...
    if (abs(vectordot(var_2327a6ed, data.axis.forward)) > 6 || vectordot(eye_dir, data.axis.forward) > -0.17) {
        return false;
    }
...
function private function_7d76b3ac() {
    ...
    while (level.var_93648050.trace_dist > 0) {
        ...
        if (!namespace_61e6d095::exists(#"hint_tutorial") && !self flag::get(#"lockpicking") && function_185fc34e(eye, eye_dir)) {
            end = eye + eye_dir * level.var_93648050.trace_dist;
            ...
            if (trace[#"fraction"] < 1) {
                ...
                for (i = start_index; i < level.var_93648050.objects.size; i++) {
                    ...
                    #/
                    if (data.max_dist > distance && [[ data.var_c42ac92 ]](data, trace, eye_dir)) {
