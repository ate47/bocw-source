...
function function_50051760() {
    ...
    var_baad2287 = spawner::simple_spawn("axis_hotel_parking_lot");
    var_baad2287 ai::waittill_dead_or_dying(var_baad2287, int(ceil(var_baad2287.size * 0.7)));
    level.woods.perfectaim = 1;
    if (var_baad2287.size) {
        function_1eaaceab(var_baad2287);
    }
    level thread namespace_42da7c51::function_d6fedf97(var_baad2287, int(ceil(var_baad2287.size)), "flag_parking_lot_complete");
    foreach (guy in var_baad2287) {
        tar = undefined;
        if (math::cointoss()) {
            tar = level.player;
        } else {
...
function function_6f9e6949() {
    ...
    var_5e907852 = [e_player, level.woods, var_89597434[0], var_89597434[1], level.var_50a8f4ca, e_door, var_6d32463a];
    level notify(#"hash_6b5c0d1e8b95af4c");
    level thread objectives::complete("hotel_breach");
    level thread scene::play("scene_tkd_hit1_hotel_room_breach", "breach", var_5e907852);
...
function function_73a5b42a() {
    ...
    while (true) {
        ...
        s_waitresult = player waittilltimeout(4, #"hash_5512f0799022267", #"ability_deactivated_camera", #"hash_4d3a7c3f614226e3");
...
function function_cebce779() {
    level endon(#"aldrich_kill_confirmed");
    level.player waittill(#"hash_4d3a7c3f614226e3");
    wait 1;
