...
function on_player_spawned()
{
    ...
    {
        var_234012ff = { #operator:"", #operator_skin:"" };
...
function function_792e6d18()
{
    foreach ( weaponinfo in self.pers[ #"weaponstats" ] )
    {
        ...
        weaponstats = { #weapon_name:weaponinfo.weapon.name, #var_5bf208a0:weaponinfo.var_6165a2d8, #shots:isdefined( weaponinfo.stats[ #"shots" ] ) ? weaponinfo.stats[ #"shots" ] : 0, #hits:isdefined( weaponinfo.stats[ #"hits" ] ) ? weaponinfo.stats[ #"hits" ] : 0, #kills:isdefined( weaponinfo.stats[ #"kills" ] ) ? weaponinfo.stats[ #"kills" ] : 0, #headshots:isdefined( weaponinfo.stats[ #"headshots" ] ) ? weaponinfo.stats[ #"headshots" ] : 0, #var_7e2968cc:isdefined( weaponinfo.stats[ #"deathsduringuse" ] ) ? weaponinfo.stats[ #"deathsduringuse" ] : 0, #time_used_s:isdefined( weaponinfo.stats[ #"timeused" ] ) ? weaponinfo.stats[ #"timeused" ] : 0, #xp_earned:isdefined( weaponinfo.stats[ #"xpearned" ] ) ? weaponinfo.stats[ #"xpearned" ] : 0, #end_level:( isdefined( self getcurrentgunrank( weaponitemindex ) ) ? self getcurrentgunrank( weaponitemindex ) : 0 ) + 1, #flourish_count:isdefined( weaponinfo.stats[ #"flourish_count" ] ) ? weaponinfo.stats[ #"flourish_count" ] : 0 };
...
function function_9cac835e( data )
{
    if ( isdefined( data.weapon ) )
    {
        update_weapon_stats( { #player:self, #weapon:data.weapon, #statname:#"flourish_count", #value:1, #weaponpickedup:0 } );
    }
}
