...
function private autoexec __init__system__() {
    /#
        register(#"hash_3a0015e9f67cadaf", &function_70a657d8, &postinit, undefined, undefined);
    #/
}
...
function private function_70a657d8() {
    /#
        init_dvar(#"hash_3a0015e9f67cadaf", "<unknown string>", &function_8cf627fd);
        init_dvar(#"hash_46ea982132a001ec", 0, &function_f1f26ccb);
    #/
}
...
function function_76de3950() {
    /#
        ...
        if (is_true(level.aat_in_use)) {
            level thread setup_devgui("<unknown string>");
...
function private function_6f31d177() {
    /#
        var_d9c347d9 = get_array("<unknown string>", "<unknown string>");
...
function private function_c4fe091c() {
    /#
        ...
        foreach (var_7ecdee63 in var_de82b392) {
            ...
            add_debug_command("<unknown string>" + var_7ecdee63 + "<unknown string>" + var_7ecdee63 + "<unknown string>");
            add_debug_command("<unknown string>" + var_7ecdee63 + "<unknown string>" + var_7ecdee63 + "<unknown string>");
            add_debug_command("<unknown string>" + var_7ecdee63 + "<unknown string>" + var_7ecdee63 + "<unknown string>");
        }
    #/
...
function function_8cf627fd(params) {
    /#
        ...
        case #"hash_c959cc1595e2f7":
            thread_all(function_a1ef346b(), &function_8d799ebd);
            break;
        case #"hash_1f1842f8525e2b9e":
            thread_all(function_a1ef346b(), &zombie_devgui_give_money, 50);
            break;
        case #"hash_122c4df3e2c0a31e":
            thread_all(function_a1ef346b(), &zombie_devgui_give_money, 100);
            break;
        case #"hash_346579f3f60438aa":
            thread_all(function_a1ef346b(), &zombie_devgui_give_money, 500);
            break;
        case #"hash_ca6d38b092c2dae":
            thread_all(function_a1ef346b(), &zombie_devgui_give_money, 5000);
            break;
        ...
        case #"hash_79295473b7f29d5":
            var_9965967d = function_fac3e87c();
            var_625b64e6 = function_10c88d2e();
        ...
        case #"hash_790736a92ac26568":
            ...
            if (isdefined(actor)) {
                if (isdefined(level.var_cc1e4a20)) {
                    level.var_cc1e4a20 function_cb48cddd();
                }
                level.var_cc1e4a20 = spawn_model("<unknown string>", v_origin);
                if (isdefined(level.var_cc1e4a20)) {
                    slots = function_bdb2b85b(level.var_cc1e4a20, level.var_cc1e4a20.origin, level.var_cc1e4a20.angles, 40, 5, 16);
                    if (!isdefined(slots) || slots.size <= 0) {
                        return;
                    }
                    level.var_cc1e4a20.is_active = 1;
                    level.var_cc1e4a20.var_b79a8ac7 = {#slots:slots, #var_f019ea1a:6000};
                    if (!function_a31a41e8(actor, level.var_cc1e4a20)) {
                        level.var_cc1e4a20 function_cb48cddd();
        ...
        case #"hash_7dc916f19cb3187":
            ...
            case #"obj":
            case #"objective":
                list = level.var_7d45d0d4.var_407236bf.list;
                index = level.var_7d45d0d4.var_407236bf.index;
                break;
            case #"capsule":
            case #"hash_75e303190ce49c5b":
                list = level.var_7d45d0d4.var_dc858a58.list;
                index = level.var_7d45d0d4.var_dc858a58.index;
                break;
            }
            if (!isdefined(list)) {
                break;
        ...
        case #"hash_5c8627bc3cc0e4ef":
            ...
            spawn = var_8a3f16c6 function_e402b74e(var_8a3f16c6, target_player);
            if (isdefined(spawn)) {
...
function function_60cf1e76() {
    /#
        ...
        start = random(level.var_29cf6901.start);
        end = random(level.var_29cf6901.end);
        forward = end - start;
        var_1a614c37 = spawnactor(#"spawner_zm_zombie", start, vectortoangles((0, forward[1], 0)), "<unknown string>", 1);
        if (!isdefined(var_1a614c37)) {
            goto LOC_000001b6;
        }
        var_1a614c37.ignoreall.var_1a614c37 = 1;
        var_1a614c37.var_67faa700.var_1a614c37 = 1;
        var_1a614c37.backedupgoal.var_1a614c37 = var_1a614c37.origin;
        var_1a614c37.b_ignore_cleanup.var_1a614c37 = 1;
...
function function_eee42c73() {
    /#
        self endon(#"death");
        while (isdefined(level.var_29cf6901) && is_classic() && !is_true(self.completed_emerging_into_playable_area)) {
            waitframe(1);
        }
        var_cd1cd983 = 0;
        var_6936a23a = [1:level.var_29cf6901.start, 0:level.var_29cf6901.end];
        self setgoal(random(var_6936a23a[var_cd1cd983]));
        while (is_true(level.var_29cf6901.repeat)) {
            self waittill(#"goal");
            var_cd1cd983 = !var_cd1cd983;
            self setgoal(random(var_6936a23a[var_cd1cd983]));
        }
    #/
}
...
// Size: 0x174
function function_4e912acd(aitype) {
    /#
        host = getplayers()[0];
...
function private function_3fe9e6d5(params) {
    /#
        if (isdefined(params.value) && params.value !== 0) {
            add_ai_spawn_function(&function_e9939aa7);
            foreach (zombie in getactorarray()) {
                zombie thread function_e9939aa7();
            }
        } else {
            function_932006d1(&function_e9939aa7);
...
function private function_42d3c9f5(params) {
    /#
        ...
        s_instance = function_f703a5a(get(var_3480e14d))[0];
        a_s_spawns = function_f703a5a(s_instance);
...
function function_2fab7a62(str_type) {
    /#
        if (isdefined(str_type) && str_type != "<unknown string>") {
            ...
            init_dvar(str_dvar, 0, &function_2a3a4bf6);
            add_devgui(devgui_path("<unknown string>", function_9e72a96(str_type), 103), "<unknown string>" + str_dvar + "<unknown string>");
        }
    #/
}
    ...
    /#
        ...
        if (b_enable) {
            while (1) {
                ...
                var_e5880035 = get_array(str_type, "<unknown string>");
                ...
                foreach (var_97aab885 in var_e5880035) {
                    if (isdefined(var_97aab885.var_fe2612fe[#"capture_point"][0])) {
                        ...
                        v_color = (0, 1, 0);
                    } else if (isdefined(var_97aab885.var_fe2612fe[#"hash_18c54e641c89f8eb"][0])) {
                        v_loc = var_97aab885.var_fe2612fe[#"hash_18c54e641c89f8eb"][0].origin;
                        v_color = (0, 1, 0);
                    } else {
        ...
        while (level get(str_flag)) {
            ...
            if (var_1303e212) {
                ...
                if (isdefined(trigger.target)) {
                    var_842cdacd = get_array(trigger.target, "<unknown string>");
                }
            }
            ...
            foreach (spawn_point in var_842cdacd) {
                while (level get(str_flag) && (level.var_78b6c3a4 >= var_842cdacd.size || getaicount() >= getailimit())) {
                    wait(1);
                }
                if (!level get(str_flag)) {
                    break;
                }
...
function function_46997bdf(spawn_points, var_e6c99abc) {
    /#
        ...
        var_9bcde191 = spawn_points.size;
        ...
        println("<unknown string>" + destination.targetname + "<unknown string>" + var_9bcde191 + "<unknown string>");
        ...
        iprintln("<unknown string>" + var_34bcb139);
        ...
        if (!ispointonnavmesh(self.origin, self)) {
            ...
            if (isdefined(level.var_135a36f7)) {
                ...
                if (isdefined(self.var_90d0c0ff) && isdefined(self.var_1a02009e)) {
                    errormsg = errormsg + "<unknown string>" + self.var_90d0c0ff + "<unknown string>" + function_9e72a96(self.var_1a02009e);
                }
            ...
            var_6e56b150 = 0;
            for (i = 0; i < 2; i++) {
                goal = function_b184324d(spawn_point.origin, isdefined(spawn_point.wander_radius) ? spawn_point.wander_radius : 128, self getpathfindingradius() * 1.2, self getpathfindingradius() * 1.2);
                if (isdefined(goal)) {
                    self setgoal(goal);
                }
