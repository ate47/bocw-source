...
function function_13d3b6eb() {
    self endon(#"damage", #"death", #"hash_3cf5b6488a20a678", #"reacting");
...
function function_a8ad2bdb() {
    ...
    level notify(#"hash_205bf17f449d4088");
    level notify(#"hash_7676a54c41c55288");
    self notify(#"hash_107bc7fb5ce914c9");
    self val::set("apartment", "ignoreall", 1);
    self val::set("apartment", "ignoreme", 1);
...
function function_3d01b9fd() {
    ...
    self endon(#"death", #"hash_107bc7fb5ce914c9");
...
function function_73f8fadd() {
    ...
    while (!level flag::get("kraus_found_corpse")) {
        ...
        if (var_5fff2a1e.size > 0) {
            foreach (corpse in var_5fff2a1e) {
                var_b1231a26 = corpse namespace_979752dc::getcorpseorigin();
                delta = var_b1231a26 - eye;
                dir = vectornormalize(delta);
                vector_dot = vectordot(dir, fwd);
                dist = distance2dsquared(var_b1231a26, eye);
                if (vector_dot > var_ed46931c && dist < var_11ecc9fe) {
                    if (bullettracepassed(eye, var_b1231a26 + (0, 0, 8), 1, self, corpse)) {
    ...
    self thread function_aa4ca19c(var_b1231a26);
...
// Size: 0x67e
function function_aa4ca19c(var_b1231a26) {
    self endon(#"damage", #"death");
    self val::set("apartment", "ignoreall", 1);
    self val::set("apartment", "ignoreme", 1);
    ...
    if (isdefined(var_b1231a26)) {
        dirtoenemy = vectornormalize(var_b1231a26 - self.origin);
    } else {
...
function function_d92bcbb5(a_ents) {
    level.var_dca3174b = a_ents[#"hash_f0dfb296354a5d4"];
}
