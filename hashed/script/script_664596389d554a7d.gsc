...
function event_handler[gametype_init] main(*eventstruct) {
    ...
    /#
        ...
        level thread function_9b928fad();
...
function function_aa5d57c6() {
    ...
    if (util::get_map_name() === #"wz_forest") {
        ...
        if (!isdefined(var_e5b9ea93)) {
            var_e5b9ea93 = {#classname:"scriptbundle_zmintel", #script_enable_on_start:1, #modelscale:1, #scriptbundlename:"zmintel_survival_omega_artifact_2", #angles:(90, 0, 48), #origin:(-4034, 799, -736)};
            ...
            var_e5b9ea93 thread namespace_4abf1500::function_23255935();
        }
    }
}
...
function function_9663b13b() {
    ...
    foreach (dynent in a_dynents) {
        if (dynent.var_15d44120 === #"hash_1247e397ea2ac1d4" && !isdefined(dynent.ondamaged)) {
            dynent.ondamaged.dynent = &namespace_f3e83343::function_c743094d;
...
function function_acafdd66() {
    ...
    while (1) {
        ...
        if (!ispointonnavmesh(self.origin, self) && !self flag::get(#"hash_7b1f9f26f086bf39")) {
            ...
            if (var_ca9a5b88 < function_a3f6cdac(500)) {
...
function function_aad874d() {
    ...
    if (util::get_map_name() === "mp_black_sea" && (is_true(level.var_53bc31ad) || level flag::get(#"gameplay_started")) && level.var_7767cea8[0].script_int === 3 && isdefined(level.var_7d45d0d4.var_49978223)) {
        var_58b02068 = zm_destination_manager::function_e4254bac(level.var_7d45d0d4.var_49978223, 1);
...
function function_e93291ff(destinations) {
    ...
    /#
        if (getdvarint(#"hash_270a21a654a1a79f", 0)) {
            ...
            foreach (destination in destinations) {
                level.var_94cbd997 = arraycombine(level.var_94cbd997, function_f703a5a(destination), 0, 0);
            }
        }
    #/
    ...
    foreach (dest in level.var_7767cea8) {
        dest.spawns.dest = namespace_8b6a9d79::function_f703a5a(dest);
    }
}
...
function on_game_playing() {
    ...
    if (!namespace_cf6efd05::function_85b812c9()) {
        ...
        } else if (!is_true(getgametypesetting(#"hash_1e1a5ebefe2772ba"))) {
...
function function_fef3deb1() {
    level flag::wait_till(#"hash_7ace2c0d668c5128");
    var_842cdacd = zm_destination_manager::function_f3be07d7(level.var_7d45d0d4.var_5f2429b1);
    var_58b02068 = struct::get(var_842cdacd[0].target, "targetname");
    level flag::set(#"hash_39574fd530246717");
    /#
        assert(isdefined(level.var_7d45d0d4.var_5f2429b1.targetname), "<unknown string>");
    #/
    /#
        assert(isdefined(var_58b02068), "<unknown string>" + level.var_7d45d0d4.var_5f2429b1.targetname);
    #/
    return var_58b02068;
}
...
function give_match_bonus() {
    ...
    foreach (player in players) {
        ...
        if (!isdefined(player.var_3b4f6b37)) {
            player.var_3b4f6b37.player = 0;
        }
        var_c602cc83 = player.var_3b4f6b37;
        var_c602cc83 = var_c602cc83 + player function_a37a8258();
...
function private function_f09350a0() {
    ...
    foreach (player in getplayers()) {
        ...
        player.var_3b4f6b37.player = player.var_3b4f6b37 + player function_a37a8258();
    }
}
    ...
    if (isarray(level.var_9ff8ac77)) {
        foreach (var_55363a21 in level.var_9ff8ac77) {
            if (isdefined(var_55363a21.v_position) && isdefined(var_55363a21.n_radius)) {
                if (distancesquared(v_destination, var_55363a21.v_position) <= function_a3f6cdac(var_55363a21.n_radius)) {
                    return 1;
                }
            }
...
function private function_d48a4cc6(*itemname, rarity) {
    ...
    if (var_a2af6935 > 0) {
        self.var_639ceafc = self.var_639ceafc + var_a2af6935;
    }
...
function private on_item_pickup(params) {
    if (is_true(getgametypesetting(#"hash_1e8998fd7f271bb7"))) {
        if (isdefined(params.item.var_a6762160.name) && isdefined(params.item.var_a6762160.rarity)) {
            var_b74300d3 = params.item.var_a6762160.name;
            rarity = params.item.var_a6762160.rarity;
            ...
            if (isalive(ai)) {
                if (var_cc6e64ae > 0) {
                    if (!isdefined(ai.var_fd23b38f)) {
                        ai.var_fd23b38f.ai = ai.health;
                    }
                } else if (isdefined(ai.var_fd23b38f)) {
                    ai.health.ai = ai.var_fd23b38f;
...
function private function_128f8f9f() {
    /#
        foreach (map in function_3cdae979()) {
            add_debug_command("<unknown string>" + map.name + "<unknown string>" + map.name + "<unknown string>");
            add_debug_command("<unknown string>" + map.name + "<unknown string>" + map.name + "<unknown string>");
        }
        add_debug_command("<unknown string>");
        add_debug_command("<unknown string>");
    ...
    /#
        foreach (map in function_3cdae979()) {
            foreach (category in level.var_4e996a3f) {
                add_debug_command("<unknown string>" + map.name + "<unknown string>" + function_9e72a96(category) + "<unknown string>" + map.name + "<unknown string>" + function_9e72a96(category) + "<unknown string>");
            }
        }
    #/
}
...
function function_447a93ab(params) {
    /#
        ...
        case #"hash_1460b3afd614fb6c":
            function_368a7cde();
            wait(1);
            level set(#"hash_1460b3afd614fb6c");
        ...
        case #"hash_defdefdefdefdef0":
            if (issubstr(params.value, "<unknown string>")) {
                foreach (player in getplayers()) {
                    player.var_5d80a93b.player = player getcurrentweapon();
                }
                cmd_tokens = strtok(params.value, "<unknown string>");
                switch (cmd_tokens.size) {
                case 3:
                    function_8e64a2b1(cmd_tokens[1], cmd_tokens[2]);
                case 2:
                    function_1aa044d3(cmd_tokens[1]);
                    break;
                case 1:
                    function_dafa6af4();
                    break;
            ...
            if (issubstr(params.value, "<unknown string>")) {
                cmd_tokens = strtok(params.value, "<unknown string>");
                objective = level.var_7d45d0d4.var_407236bf.list[level.var_7d45d0d4.var_407236bf.index];
                function_8e64a2b1(cmd_tokens[1], objective);
            }
            break;
        }
    #/
