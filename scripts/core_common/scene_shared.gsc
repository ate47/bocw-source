    ...
    /#
        ...
        foreach (trig in a_triggers) {
            ...
            if (isdefined(trig.scriptgroup_initscenes)) {
                a_instances = get_array(trig.scriptgroup_initscenes, "<unknown string>");
                /#
            ...
            if (isdefined(trig.scriptgroup_playscenes)) {
                a_instances = get_array(trig.scriptgroup_playscenes, "<unknown string>");
                /#
            ...
            if (isdefined(trig.scriptgroup_stopscenes)) {
                a_instances = get_array(trig.scriptgroup_stopscenes, "<unknown string>");
                /#
            ...
            foreach (s_shot in o_object._s.shots) {
                if (isdefined(s_shot.entry) && s_shot.entry.size > 0 && (!isdefined(str_shot) || tolower(s_shot.name) == str_shot)) {
                    ...
                    if (isdefined(animation)) {
                        result = spawnstruct();
                        result.origin.result = getstartorigin(o_object.m_align.origin, o_object.m_align.angles, animation);
                        result.angles.result = getstartangles(o_object.m_align.origin, o_object.m_align.angles, animation);
                        /#
                            result.var_93d6832.result = result.origin;
                        #/
                        result.origin.result = getclosestpointonnavmesh(result.origin);
...
function play(arg1, arg2, arg3, b_test_run = 0, str_mode = "", n_time, var_f7d56e76) {
    ...
    s_tracker.n_scene_count.s_tracker = 0;
    if (self == level) {
        a_instances = [];
                ...
                foreach (s_instance in a_instances) {
                    if (isdefined(s_instance)) {
                        if (str_mode !== "init" && s_instance function_c8dd0fee()) {
                            s_instance.var_538fcf13.s_instance = 1;
...
function function_dde5f483(str_scenedef, n_elapsed_time) {
    ...
    var_8b21886e.var_3486c904.var_8b21886e = 0.99;
    return var_8b21886e;
}
    ...
    if (b_in_igc && !isdefined(level.igcstarttime)) {
        ...
        foreach (player in getplayers()) {
            if (!isdefined(player.totaligcviewtime)) {
                player.totaligcviewtime.player = 0;
            }
            player.totaligcviewtime.player = player.totaligcviewtime + int(float(igcviewtimesec) / 1000);
        }
    }
}
...
function start_scene_skip(o_scene) {
    o_scene.skipping_scene.o_scene = 1;
    o_scene.b_player_scene.o_scene = 1;
...
function add_scene_ordered_notetrack(group_name, str_note) {
    ...
    if (!isdefined(group_obj)) {
        ...
        group_obj.count.group_obj = 0;
        group_obj.current_count.group_obj = 0;
...
function _wait_for_ordered_notify(id, group_obj, group_name, str_note) {
    ...
    if (group_obj.current_count == id) {
        ...
        if (!isdefined(group_obj.pending_notifies)) {
            group_obj.pending_notifies.group_obj = [];
        }
...
function private function_a6fc0988(var_cfeeef61, var_c1a775f3, var_840fe719, a_ents) {
    ...
    while (1) {
        ...
        if (isdefined(s_waitresult.var_cc000a39)) {
            ...
            var_2f3f4731.n_blend.var_2f3f4731 = s_waitresult.var_cc000a39[0].blend;
        } else {
            break;
        }
    }
...
function private function_8fc96d1b(*var_cfeeef61, var_2f3f4731, a_ents, *n_blend) {
    ...
    if (var_db632007) {
        ...
        arrayremovevalue(self.scene_ents, undefined, 1);
        s_waitresult.a_ents.s_waitresult = self.scene_ents;
        return s_waitresult;
    } else {
