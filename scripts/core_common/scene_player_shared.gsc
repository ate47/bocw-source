...
class class_6572d7cd : csceneplayer, csceneobject {
    ...
    // Checksum 0xa89cf4bc, Offset: 0xf258
    // Size: 0x14
    __constructor() {
    ...
    // Checksum 0x891ae302, Offset: 0xf278
    // Size: 0x14
    __destructor() {
...
class csceneplayer : csceneobject {
    ...
    // Namespace csceneplayer/scene_player_shared
    // Params 0, eflags: 0x92 linked class_linked
    // Checksum 0x4a4670ed, Offset: 0x88b0
    // Size: 0x14
    __destructor() {
    ...
    function function_607d830() {
        return is_true(self._s.hidestowedweapon) || is_true(self.var_55b4f21e.hidestowedweapon);
    }
    ...
    function function_d09b043() {
        if (is_true(self.var_55b4f21e.interactiveshot)) {
            return;
        }
    ...
    function function_1db23c62() {
        return is_true(self._s.var_1a95895f) || is_true(self.var_55b4f21e.var_1a95895f);
    }
    ...
    function play_camera(animation, n_start_time = 0) {
        ...
        if (csceneobject::is_shared_player()) {
            ...
            if (!a_players.size && is_true(self._o_scene._b_testing)) {
                var_169da9db = util::get_enemy_team(self._o_scene._str_team);
                a_players = [[ self._func_get ]](var_169da9db);
            }
        ...
        /#
            if (getdvarint(#"debug_scene", 0) > 0) {
                log(toupper(self._s.type) + "<unknown string>" + self._str_camera + "<unknown string>");
            }
        #/
    ...
    function _play_anim(animation, n_rate, n_blend, var_b2e32ae2, n_time, var_7d32b2c6, paused) {
        ...
        if (csceneobject::function_5c2a9efa()) {
            ...
            if (isdefined(player.var_d43badce)) {
                player.var_d43badce animation::play(animation, self.m_align, self.m_tag, n_rate, n_blend, 0, n_lerp, n_time, self._s.showweaponinfirstperson, b_unlink_after_completed, self.var_f4b34dc1, paused, undefined, var_b2e32ae2);
            } else {
                player animation::play(animation, self.m_align, self.m_tag, n_rate, n_blend, 0, n_lerp, n_time, self._s.showweaponinfirstperson, b_unlink_after_completed, self.var_f4b34dc1, paused, undefined, var_b2e32ae2, self._s.showviewmodel);
            }
    ...
    function function_247a0b9d() {
        return is_true(self._s.var_b538acac) || is_true(self.var_55b4f21e.var_b538acac);
    }
    ...
    function animation_lookup(animation, ent = self._e, b_camera = 0) {
        if (isdefined(self._o_scene.var_58e5d094) && !b_camera) {
            if (self === self._o_scene.var_58e5d094 && isarray(self._o_scene._a_objects)) {
                foreach (obj in self._o_scene._a_objects) {
                    if (obj != self && [[ obj ]]->function_527113ae() && isdefined(obj._e)) {
                        var_65cfb30b = [[ obj ]]->get_animation_name(self._str_shot, 1);
                        var_65cfb30b = [[ obj ]]->function_9a43e31(ent, var_65cfb30b);
                        self.var_f4b34dc1[obj._e getentitynumber()] = var_65cfb30b;
                    }
                }
            }
            var_9defbbb0 = [[ self._o_scene.var_58e5d094 ]]->get_animation_name(self._str_shot, 1);
            var_9defbbb0 = [[ self._o_scene.var_58e5d094 ]]->function_9a43e31(ent, var_9defbbb0);
            self.var_f4b34dc1[ent getentitynumber()] = var_9defbbb0;
            if (is_true(self._s.var_186d089d) && isarray(self._o_scene._a_objects)) {
                foreach (obj in self._o_scene._a_objects) {
                    if (obj != self && [[ obj ]]->function_527113ae() && isplayer(obj._e) && isplayer(ent) && !is_true(obj._s.var_186d089d)) {
                        var_a2265eca = [[ obj ]]->get_animation_name(self._str_shot, 1);
                        var_8d34c269 = csceneobject::get_animation_name(self._str_shot, 1);
    ...
    function function_509b9f47(player) {
        ...
        if (!scene::function_6a0b0afe(self._o_scene._str_mode)) {
    ...
    function _set_values(ent = self._e) {
        if (!is_true(self._s.takedamage) && !sessionmodeismultiplayergame() && !sessionmodeiswarzonegame() && !sessionmodeiszombiesgame()) {
        ...
        csceneobject::set_ent_val("ignoreme", !is_true(self._s.takedamage), ent);
    ...
    function _cleanup_player(player) {
        ...
        if (isplayer(player) && (csceneobject::function_b260bdcc(self._str_shot) || scene::function_6a0b0afe(self._o_scene._str_mode) || is_true(self._o_scene.scene_stopping))) {
            player scene::set_igc_active(0);
            player notify(#"hash_684b272680aa2ed", {#str_scene:self._o_scene._str_name});
        ...
        if (is_true(player.var_20ed0b0c) && (csceneobject::function_b260bdcc(self._str_shot) || scene::function_6a0b0afe(self._o_scene._str_mode) || is_true(self._o_scene.scene_stopping))) {
            player delete();
        }
        /#
    ...
    function display_dev_info() {
        /#
            if (isstring(self._o_scene._s.devstate) && getdvarint(#"scr_show_shot_info_for_igcs", 0)) {
                ...
                if (!isdefined(self._o_scene.var_9d90ef8b)) {
        ...
        while (1) {
            ...
            if (is_true(var_ec50a0d3.var_769fadce)) {
                ...
                if (isanimlooping(self.var_efc540b6)) {
                    var_912e843b = util::spawn_player_clone(player, self.var_efc540b6, csceneobject::function_bc0facbb().ent, 1);
                ...
                if (!var_1c45c7f8) {
                    if (isanimlooping(self.var_efc540b6)) {
                        thread _play_anim(self.var_efc540b6, 1, 0.2, 0);
                    } else {
                        _play_anim(self.var_efc540b6, 1, 0.2, 0);
                    }
                }
                var_a3cc5416 = 1;
            ...
            if (var_a3cc5416 >= 1 || var_a3cc5416 <= 0 && csceneobject::function_a808aac7()) {
                if (csceneobject::function_a808aac7()) {
                    ...
                    if (var_778409ab > 0 && isdefined(self._s.shots[var_778409ab - 1]) && csceneobject::function_e91c94b9(var_778409ab - 1) && !is_true(self._s.shots[var_778409ab - 1].var_751fe16b)) {
                        self.var_a3cc5416 = 1;
    ...
    function _stop(*b_dont_clear_anim, b_finished) {
        if (isalive(self._e)) {
            ...
            self._e notify(#"hash_7ba9e3058f933eb");
            stop_camera(self._e);
            self._e flag::clear(#"hash_7cddd51e45d3ff3e");
            if (!is_true(self._s.diewhenfinished) || !b_finished) {
                self._e animation::stop(0.2);
            }
            self._e thread scene::scene_enable_player_stuff(self._o_scene._s, self._s, self._o_scene._e_root);
        }
        if (is_true(self._e.var_20ed0b0c)) {
            self._e delete();
        }
    }
    ...
    function function_9339d69d(player) {
        if (is_true(self._s.var_6d72caf2) && !csceneobject::function_27898329(self._str_current_anim) && (csceneobject::function_b260bdcc(self._str_shot) || scene::function_6a0b0afe(self._o_scene._str_mode))) {
            self notify("4f895ebd7b592448");
            self endon("4f895ebd7b592448");
            player endon(#"disconnect");
            if (isdefined(self._str_current_anim)) {
                n_anim_length = getanimlength(self._str_current_anim);
            } else if (isdefined(self._str_camera)) {
                n_anim_length = float(getcamanimtime(self._str_camera)) / 1000;
            } else {
                n_anim_length = 0;
            }
            n_wait = max(n_anim_length - 0.5, 0);
            self._o_scene waittilltimeout(n_wait, #"scene_done", #"scene_stop", #"scene_skip_completed", #"hash_3168dab591a18b9b");
    ...
    function play_camera_on_player(player, n_start_time) {
        if (!isplayer(player) || !scene::check_team(player.team, self._str_team) && !is_true(self._o_scene._b_testing)) {
            return;
        }
        csceneobject::update_alignment();
        v_origin = isdefined(self.m_align.origin) ? self.m_align.origin : (0, 0, 0);
        v_angles = isdefined(self.m_align.angles) ? self.m_align.angles : (0, 0, 0);
    ...
    function _prepare_player(player) {
        ...
        var_2bb59a6a = csceneobject::function_24f8cfb5(self._str_shot) || is_true(self._o_scene.b_play_from_time);
        function_dd8a2de2(player);
        if (player.current_scene === self._o_scene._str_name) {
            return 0;
        }
        /#
            if (player === level.host && check_team(player.team, self._str_team)) {
                display_dev_info();
            }
        #/
        if (isplayer(player) && !is_true(self._s.showhud) && !is_true(self._s.allowdeath) && !player scene::is_igc_active()) {
            player scene::set_igc_active(1);
            player notify(#"scene_igc_shot_started", {#str_scene:self._o_scene._str_name});
        }
        ...
        if (player.team === #"allies" || player.team === #"axis") {
            ...
            str_streamer_hint = self._o_scene._a_streamer_hint[util::get_team_mapping(#"sidea")];
        }
        if (isdefined(str_streamer_hint) && isplayer(player)) {
            ...
            if (var_2bb59a6a && self._n_streamer_req != -1) {
                if (!is_true(level.scene_streamer_ignore[self._o_scene._s.name])) {
                    if (!is_true(self._o_scene._s.ignorestreamer) && !is_true(self._o_scene._b_testing) && !scene::function_46546b5c(self._o_scene._s.name)) {
                        self endon(#"new_shot");
        ...
        #/
        if (isplayer(player) && [[ self._o_scene ]]->is_scene_shared() && (var_2bb59a6a || scene::function_46546b5c(self._o_scene._str_name)) && !csceneobject::is_skipping_scene()) {
        ...
        } else if (isalive(player) && !is_true(self._s.var_5829ce6b) && isplayer(player) && player isinvehicle()) {
            vh_occupied = player getvehicleoccupied();
            n_seat = vh_occupied getoccupantseat(player);
            vh_occupied usevehicle(player, n_seat);
        }
        if (self._o_scene._s scene::is_igc() && isplayer(player)) {
        ...
        if (is_true(self.var_55b4f21e.var_3a0a7985) && isplayer(player)) {
            function_a2a183e7(player, self.var_55b4f21e.var_6d62f1ae);
        }
        if (is_true(self._s.var_186d089d)) {
            player setinvisibletoall();
            player setvisibletoplayer(player);
        }
        waittillframeend();
        if (is_true(self.var_55b4f21e.interactiveshot) && isplayer(player)) {
    ...
    function function_d3541c6f(player) {
        ...
        if (isdefined(player.var_777951c)) {
            if (!is_true(self._s.var_1a95895f) && !is_true(self._s.var_b538acac) || (is_true(self._s.var_1a95895f) || is_true(self._s.var_b538acac)) && (scene::function_6a0b0afe(self._o_scene._str_mode) || scene::function_b260bdcc(self._o_scene._str_name, self._str_shot) || is_true(self._o_scene.scene_stopping))) {
        ...
        /#
            if (isdefined(player.var_96494502) && (function_6a0b0afe(self._o_scene._str_mode) || function_b260bdcc(self._o_scene._str_name, self._str_shot))) {
        ...
        if (!is_true(self._s.dontreloadammo) && (!isdefined(w_current.var_a2e7031d) || !w_current.var_a2e7031d)) {
    ...
    function function_d46ffe56(player) {
        ...
        if ((!is_true(self._s.removeweapon) || is_true(self._s.showweaponinfirstperson)) && !is_true(self._s.disableprimaryweaponswitch) && var_cd31cd6a == "" && !function_1db23c62()) {
            player player::switch_to_primary_weapon(1, 1);
        }
    ...
    function get_camera_tween_out() {
        n_camera_tween_out = isdefined(self.var_55b4f21e.cameratweenout) ? self.var_55b4f21e.cameratweenout : self._s.cameratweenout;
    ...
    function _spawn() {
        /#
            if (is_true(self._o_scene._b_testing) && is_player()) {
                p_host = gethostplayer();
        ...
        if (!isdefined(self._e)) {
            ...
            if (!isdefined(self._e) && is_true(self._s.var_9b604b8d)) {
                ...
                if (isdefined(player)) {
                    ...
                    self._e.var_777951c = current_weapon;
                    self._e.var_3b97696d = player getplayergendertype();
                }
            }
        }
    }
    ...
    function _reset_values(ent = self._e) {
        ...
        if (isbot(ent) && (csceneobject::function_b260bdcc(self._str_shot) || scene::function_6a0b0afe(self._o_scene._str_mode) || is_true(self._o_scene.scene_stopping))) {
    ...
    function on_play_anim(player) {
        ...
        if (self._n_streamer_req != -1 && !csceneobject::is_skipping_scene() && !is_true(player.streamer_hint_playing)) {
...
class cscenefakeplayer : csceneobject {
    ...
    // Params 0, eflags: 0x1a linked
    // Checksum 0xc14c4737, Offset: 0xdc90
    // Size: 0x14
    __constructor() {
        csceneobject::__constructor();
    }
    ...
    // Namespace cscenefakeplayer/scene_player_shared
    // Params 0, eflags: 0x92 linked class_linked
    // Checksum 0x6cbeac12, Offset: 0xdcb0
    // Size: 0x14
    __destructor() {
        csceneobject::__destructor();
    }
    ...
    // Size: 0x92
    function _spawn_ent() {
        if (is_true(self._s.var_671e284a)) {
    ...
    function animation_lookup(animation, ent, *b_camera = self._e) {
        ...
        if (isdefined(self._o_scene.var_58e5d094) && isplayer(b_camera.var_1ff8de20)) {
            ...
            var_9defbbb0 = [[ self._o_scene.var_58e5d094 ]]->get_animation_name(self._str_shot, 1);
            var_9defbbb0 = [[ self._o_scene.var_58e5d094 ]]->function_9a43e31(b_camera, var_9defbbb0);
    ...
    function _prepare() {
        /#
            ...
            if (isdefined(self._e) && var_cd31cd6a != "<unknown string>") {
                weapon = getweapon(var_cd31cd6a);
                self._e attach_weapon(weapon);
            }
        #/
...
class cscenesharedplayer : csceneplayer, csceneobject {
    ...
    // Params 0, eflags: 0x1a linked
    // Checksum 0xa42928c, Offset: 0xb748
    // Size: 0x14
    __constructor() {
        csceneplayer::__constructor();
    }
    ...
    // Namespace cscenesharedplayer/scene_player_shared
    // Params 0, eflags: 0x92 linked class_linked
    // Checksum 0xc2607f85, Offset: 0xb768
    // Size: 0x14
    __destructor() {
        csceneplayer::__destructor();
    }
    ...
    function _play_anim(animation, n_rate, *n_blend, *var_b2e32ae2, n_time) {
        ...
        do {
            ...
            foreach (player in [[ self._func_get_active ]](self._str_team)) {
                if (isdefined(player) && player flag::get(self.player_animation_notify)) {
                    b_playing = 1;
                    player flag::wait_till_clear(self.player_animation_notify);
                    break;
                }
            }
        } while(b_playing);
    ...
    function _play_shared_player_anim_for_player(player) {
        ...
        if (player flag::get(self.player_animation_notify)) {
            player flag::set(self.player_animation_notify + "_skip_init_clear");
        }
        player flag::set(self.player_animation_notify);
        if (isdefined(player getlinkedent())) {
            player unlink();
        }
        if (!is_true(self._s.disabletransitionin)) {
        ...
        if (n_time_frac < 1) {
            ...
            if (csceneobject::function_5c2a9efa()) {
                ...
                player animation::play(str_animation, self.player_align, self.player_tag, self.player_rate, 0, 0, n_lerp, n_time_frac, self._s.showweaponinfirstperson, undefined, undefined, undefined, undefined, undefined, self._s.showviewmodel);
            }
            if (!player flag::get(self.player_animation_notify + "_skip_init_clear")) {
                player flag::clear(self.player_animation_notify);
            } else {
                player flag::clear(self.player_animation_notify + "_skip_init_clear");
            }
            ...
            /#
                if (getdvarint(#"debug_scene", 0) > 0) {
                    printtoprightln("<unknown string>" + self._s.name + "<unknown string>" + function_9e72a96(self.player_animation));
                }
            #/
        }
    }
