...
function function_8c88a649(localclientnum, *oldval, newval, *bnewent, *binitialsnap, *fieldname, *bwasdemojump) {
    ...
    if (bwasdemojump) {
        if (!isdefined(e_portal.var_b875cdb1)) {
            e_portal.var_b875cdb1.e_portal = playfx(fieldname, #"hash_532aade47be565ff", e_portal.origin, anglestoforward(e_portal.angles), anglestoup(e_portal.angles));
        }
...
function function_2d49baf(localclientnum, *oldval, newval, *bnewent, *binitialsnap, *fieldname, *bwasdemojump) {
    ...
    if (bwasdemojump) {
        if (bwasdemojump == 1) {
            ...
            namespace_ff7e490::function_60381056(fieldname, e_portal);
            e_portal.var_b875cdb1.e_portal = playfx(fieldname, #"hash_9d2f5030c333b16", e_portal.origin, anglestoforward(e_portal.angles), anglestoup(e_portal.angles));
            e_portal.var_a3b04735.e_portal = e_portal playloopsound(#"hash_83b5ecd7e3f8f29");
...
function private function_d93b0fff(localclientnum, e_portal) {
    if (!isdefined(e_portal.var_b875cdb1)) {
        e_portal.var_b875cdb1.e_portal = playfx(localclientnum, #"hash_42ec8f4ec8bcc91b", e_portal.origin, anglestoforward(e_portal.angles), anglestoup(e_portal.angles));
    }
    if (!isdefined(e_portal.var_bd434ca)) {
        e_portal.var_bd434ca.e_portal = playfx(localclientnum, #"hash_22568d89d3063dec", e_portal.origin, anglestoforward(e_portal.angles), anglestoup(e_portal.angles));
    }
    if (!isdefined(e_portal.var_a3b04735)) {
        e_portal playsound(localclientnum, #"hash_c2c4f6486acdcd6");
        e_portal.var_a3b04735.e_portal = e_portal playloopsound(#"hash_3a1b6a329b7e308f");
    }
}
