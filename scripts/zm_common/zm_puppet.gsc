...
function wait_for_puppet_pickup() {
    /#
        ...
        while (true) {
            ...
            if (is_true(self.ispuppet) && check_point_in_playable_area(self.origin) && !isdefined(self.completed_emerging_into_playable_area)) {
            ...
            if (isdefined(player) && player buttonpressed("<unknown string>")) {
                if (self.iscurrentlypuppet) {
                    if (check_point_in_playable_area(self.goalpos) && !check_point_in_playable_area(self.origin)) {
                        self.backedupgoal = self.goalpos;
                    }
                    if (!check_point_in_playable_area(self.goalpos) && isdefined(self.backupnode) && self.goalpos != self.backupnode.origin) {
                        self notify(#"stop_zombie_goto_entrance");
                    }
                }
            }
