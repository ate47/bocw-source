...
function function_6c2b3729() {
    /#
        add_custom_devgui_callback(&function_2f63dc81);
        ...
        if (isdefined(level.a_s_defend_areas)) {
            ...
            foreach (var_355646ad in var_f95f5bc7) {
                ...
                add_debug_command("<unknown string>" + str_name + "<unknown string>" + "<unknown string>" + str_name + "<unknown string>" + "<unknown string>");
                add_debug_command("<unknown string>" + str_name + "<unknown string>" + "<unknown string>" + str_name + "<unknown string>" + "<unknown string>");
            }
        }
    #/
    ...
    /#
        ...
        case #"hash_196a879b48e37805":
            if (function_880bd896()) {
                drop_key(level.players[0].origin);
            }
            break;
        case #"hash_2dd0872d741b071e":
            zombie_devgui_give_powerup("<unknown string>", undefined, level.players[0].origin);
            break;
            ...
            if (isdefined(level.a_s_defend_areas)) {
                ...
                if (isinarray(var_f95f5bc7, hash(str_name))) {
                    if (var_19d45c79 === "<unknown string>") {
                        ...
                        s_defend_area = function_a877cd10(str_name);
                        level.var_9149f3ac = s_defend_area.var_ed1db1a7;
                        level.var_35da2d77 = function_d7db256e(level.var_9149f3ac, #"hash_683cf7d37afcc3ae");
                    } else {
                        iprintlnbold("<unknown string>" + str_name + "<unknown string>");
                        s_defend_area = function_a877cd10(str_name);
                        function_33798535(s_defend_area.var_39c44288, s_defend_area.a_str_zones, s_defend_area.var_ed1db1a7, undefined, undefined, 30);
                    }
                }
            }
            break;
    ...
    while (1) {
        if (level.players.size == 1) {
            ...
            if (player.var_7e008e0c >= 75) {
                player.var_cf914505.player = 0.01;
                player.var_72b24dc2.player = 15 - 11.25;
            } else if (level.players[0].var_7e008e0c >= 50) {
                player.var_cf914505.player = 0.00875;
                player.var_72b24dc2.player = 15 - 7.5;
            } else if (level.players[0].var_7e008e0c >= 25) {
                player.var_cf914505.player = 0.0075;
                player.var_72b24dc2.player = 15 - 3.75;
            } else {
                player.var_cf914505.player = 0.005;
                player.var_72b24dc2.player = 15;
            }
            if (player.var_72b24dc2 <= 5) {
                player.var_72b24dc2.player = 6;
            }
            wait(1);
            continue;
        }
        ...
        foreach (player in level.players) {
            ...
            if (level.score_total > 0) {
                player.var_4b6aee2b.player = player.score_total / level.score_total;
            }
...
function function_cd6476e(var_16ef1c43) {
    ...
    /#
        if (self get_client_stat(#"hash_47685630580f6b5f") >= 100000000) {
...
function function_a79dd957(var_f2c28d86, str_event, *ai_killed) {
    ...
    if (ai_killed === "bonus_points_player_grabbed" || ai_killed === "player_grabbed_key") {
        ...
        foreach (ai_zombie in a_ai_zombies) {
            if (level.round_number >= var_f235658b) {
                if (!is_true(ai_zombie.var_eceaa835)) {
                    ai_zombie.var_eceaa835.ai_zombie = 1;
            ...
            } else if (n_time >= 30 && !is_true(ai_zombie.var_497d2f90)) {
                ai_zombie.var_497d2f90.ai_zombie = 1;
                if (math::cointoss(5)) {
                    ai_zombie thread zombie_utility::set_zombie_run_cycle("sprint");
                } else {
...
function function_fe4381f8() {
    foreach (s_barricade in level.exterior_goals) {
        s_barricade.script_delay.s_barricade = float(function_60d95f53()) / 1000;
    }
}
...
function function_127f0ac5(e_player) {
    ...
    if (n_cooldown > 0) {
        ...
        e_player.var_838c00de.e_player = 1;
...
function function_51e47b48(var_e5fb46a8, str_name, n_time_ms) {
    if (!isdefined(var_e5fb46a8.var_bde53893)) {
        var_e5fb46a8.var_bde53893.var_e5fb46a8 = [];
    }
...
function function_7acf9d9b(e_door) {
    if (zm_utility::function_e37823df() && zm_utility::function_5f8f4d1b(e_door) && !is_true(e_door.var_c947f134)) {
        ...
        return 1;
    } else if (self == e_door && !is_true(e_door.var_c947f134)) {
        return 1;
    } else {
...
function function_b8839207(e_door) {
    ...
    } else if (e_door.script_noteworthy === "electric_door" || e_door.script_noteworthy === "electric_buyable_door" || is_true(e_door.var_c947f134)) {
