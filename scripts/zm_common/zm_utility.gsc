...
function private function_70a657d8() {
    ...
    /#
        init_dvar(#"hash_26af2f9714c95db1", 0, &function_452938ed);
    #/
...
function move_zombie_spawn_location(spot) {
    ...
    foreach (token in tokens) {
        ...
        if (token == "custom_spawner_entry") {
            next_token = index + 1;
            jumpiffalse(isdefined(tokens[next_token])) LOC_0000023c;
            str_spawn_entry = tokens[next_token];
            jumpiffalse(isdefined(level.custom_spawner_entry) && isdefined(level.custom_spawner_entry[str_spawn_entry])) LOC_0000023c;
        ...
        if (!tracepassedonnavmesh(var_7162cf15, queryresult.data[i].origin, 15)) {
            /#
                if (is_true(level.var_565d6ce0)) {
                    ...
                    record3dtext("6d99fc4675da1628", queryresult.data[i].origin + vectorscale((0, 0, 1), 8), (1, 0, 0));
                }
            #/
        } else {
...
function get_zombie_point_of_interest(origin, poi_array) {
    ...
    if (isdefined(best_poi)) {
        ...
        if (is_true(best_poi._new_ground_trace)) {
            var_a9740555 = {};
            var_a9740555.position.var_a9740555 = groundpos_ignore_water_new(best_poi.origin + vectorscale((0, 0, 1), 100));
            var_a9740555.poi_ent = best_poi;
        } else if (isdefined(best_poi.attract_to_origin) && best_poi.attract_to_origin) {
            var_a9740555 = {};
            var_a9740555.position.var_a9740555 = groundpos(best_poi.origin + vectorscale((0, 0, 1), 100));
            var_a9740555.poi_ent = best_poi;
        } else {
...
function add_poi_attractor(zombie_poi) {
    ...
    if (!isinarray(zombie_poi.attractor_array, self)) {
        ...
        if (!isdefined(zombie_poi.attractor_array)) {
            ...
            zombie_poi.attractor_array.zombie_poi = array(zombie_poi.attractor_array);
        }
        ...
        if (!isdefined(zombie_poi.claimed_attractor_positions)) {
            ...
            zombie_poi.claimed_attractor_positions.zombie_poi = array(zombie_poi.claimed_attractor_positions);
        }
        ...
        if (reset_closest_player) {
            foreach (zombie in zombies) {
                if (isdefined(zombie.need_closest_player)) {
                    zombie.need_closest_player.zombie = 1;
                    /#
...
function function_c52e1749(origin, players) {
    ...
    var_88e86621.height.var_88e86621 = self function_6a9ae71();
    var_88e86621.radius.var_88e86621 = self getpathfindingradius();
    ...
    if (isdefined(self.var_6392b6c4)) {
        ...
        foreach (player in players) {
            ...
            if (!isdefined(player_pos)) {
                ...
                jumpiftrue(isdefined(player_pos)) LOC_00000368;
            } else {
            LOC_00000368:
        ...
        if (isdefined(pathdata) && pathdata.status === "succeeded") {
            ...
            if (getdvarint(#"hash_4477ab37a00b1492", 1) == 1) {
                ...
                foreach (position in playerpositions) {
                    ...
                    } else if (distancesquared(position, goalpos) < function_a3f6cdac(16)) {
                        closestplayer = players[index];
                        break;
                    }
                }
...
function function_3d70ba7a(origin, players) {
    ...
    if (isdefined(self.var_6392b6c4)) {
        ...
        foreach (player in players) {
            ...
            if (!isdefined(player_pos)) {
                player_pos = getclosestpointonnavmesh(player.origin, 100, var_88e86621.radius);
                jumpiftrue(isdefined(player_pos)) LOC_00000438;
            } else {
            LOC_00000438:
                if (var_88e86621.radius > player getpathfindingradius()) {
                ...
                if (!isdefined(self.var_2d5cbb7[player getentitynumber()])) {
                    self.var_2d5cbb7[player getentitynumber()] = {#dist:-1, #var_e29d2657:0};
                }
                position_info = {#origin:pos, #player:player};
                if (!isdefined(var_448ee423)) {
                    var_448ee423 = [];
    ...
    if (ispointonnavmesh(var_88e86621.origin, self)) {
        ...
        var_8aa6bded = -1;
        var_e3fc4816 = undefined;
        for (i = 0; i < var_674755ca.size; i++) {
            if (var_674755ca[i] != -1) {
                if (!isdefined(var_e3fc4816) || var_8aa6bded > var_674755ca[i]) {
                    var_e3fc4816 = var_448ee423[i].player;
                    var_8aa6bded = var_674755ca[i];
                }
            }
        }
        if (!isdefined(var_e3fc4816)) {
            return undefined;
        }
        for (i = 0; i < var_674755ca.size; i++) {
            ...
            if (isdefined(self.var_448aebc7[potential_target getentitynumber()])) {
                ...
                var_e3958ef0[i] = var_e3958ef0[i] - function_a3f6cdac(var_ab0d150d * var_7e10832f);
            }
        ...
        if (!is_true(self.var_d8861a5f)) {
            if (!is_true(self.var_982e6932)) {
                ...
                self.last_closest_player = var_e3fc4816;
            ...
            if (isdefined(self.var_c2dcab66)) {
                ...
                foreach (attacker in self.var_c2dcab66) {
                    ...
                    if (isdefined(attacker.player) && isdefined(self.var_448aebc7[attacker.player getentitynumber()])) {
                        var_edbf2d06 = var_edbf2d06 + "<unknown string>" + attacker.player getentitynumber() + "<unknown string>" + self.var_448aebc7[attacker.player getentitynumber()] + "<unknown string>" + var_76fc2ac9 + "<unknown string>";
                    }
                }
                record3dtext(var_edbf2d06, self.origin + vectorscale((0, 0, -1), 20), (1, 1, 1), "<unknown string>", self, 0.5);
            }
            if (is_true(self.var_982e6932)) {
                record3dtext("<unknown string>", self.origin + vectorscale((0, 0, -1), 25), (0, 1, 0), "<unknown string>", self, 0.5);
            }
        }
    #/
    self function_5dcc54a8(players);
...
function get_closest_valid_player(origin, ignore_player = array(), var_b106b254 = 0) {
    ...
    if (isdefined(player.last_valid_position)) {
        ...
        if (distance2dsquared(self.origin, player.last_valid_position) < function_a3f6cdac(self getpathfindingradius()) && var_171dce2 > var_1029d6d2 * 0.25 && var_171dce2 < var_1029d6d2 || abs(self.origin[2] - player.last_valid_position[2]) < var_1029d6d2 * 0.25) {
            var_3bd0e427 = self.origin[2];
        }
    }
    var_3d3d6684 = length(player getvelocity());
    if (!(isdefined(player.last_valid_position) && isdefined(var_3bd0e427)) || !isdefined(player getgroundent()) && !player isonladder() && var_3d3d6684 != 0 || distancesquared(player.last_valid_position, player.origin) < (isdefined(self.var_154478e3) ? self.var_154478e3 : function_a3f6cdac(var_3d3d6684 + 45)) && abs(var_3bd0e427 - player.origin[2]) < (isdefined(self.var_737e8510) ? self.var_737e8510 : level.var_376e688) && (is_true(level.var_9f01688e) || !is_true(player.cached_zone_volume.var_8e4005b6))) {
...
// Size: 0xd8
function spawn_weapon_model(weapon, model = weapon.worldmodel, origin, angles, var_57fe74e5, var_fd90b0bb) {
        ...
        /#
            foreach (player in players) {
                if (!isdefined(player.var_ab8c5e97)) {
                    player.var_ab8c5e97.player = [];
                }
                if (!isdefined(player.var_d28c72e5)) {
                    player.var_d28c72e5.player = 1000;
                }
            }
        #/
        var_93bba48c = [];
        for (i = 0; i < players.size; i++) {
            ...
            for (j = 0; j < players.size; j++) {
                ...
                if (isdefined(level.player_intersection_tracker_override)) {
                    jumpiffalse(players[i] [[ level.player_intersection_tracker_override ]](players[j])) LOC_00000440;
                } else {
                LOC_00000440:
...
function spawn_path_node(origin, angles, k1, v1, k2, v2) {
    ...
    node.k2 = k2;
    node.v2 = v2;
    node.node.node = spawn_path_node_internal(origin, angles, k1, v1, k2, v2);
...
function respawn_path_nodes() {
    ...
    for (i = 0; i < level._spawned_path_nodes.size; i++) {
        ...
        #/
        node_struct.node.node_struct = spawn_path_node_internal(node_struct.origin, node_struct.angles, node_struct.k1, node_struct.v1, node_struct.k2, node_struct.v2);
    }
}
...
function function_f5a222a8(var_6cc77d4e, v_origin_or_ent, var_c87f9ad7, params) {
    ...
    if (!isdefined(level.var_e63703cd[n_obj_id])) {
        level.var_e63703cd[n_obj_id] = {#params:params, #var_c87f9ad7:var_c87f9ad7, #v_origin_or_ent:v_origin_or_ent, #var_6cc77d4e:var_6cc77d4e};
    }
    return n_obj_id;
}
...
function function_ff902863(var_6cc77d4e = level.var_e63703cd[n_obj_id].var_6cc77d4e, n_obj_id, var_3a9f00e9 = 1) {
    if (var_3a9f00e9) {
        s_objective = {#var_3dce3470:function_f8d53445(), #var_f81e2f33:n_obj_id, #var_f23c87bd:var_6cc77d4e};
    } else {
        s_objective = {#var_84820801:function_f8d53445(), #var_9a059624:n_obj_id, #var_81d2684e:var_6cc77d4e};
    }
    ...
    foreach (ai in getaiteamarray(level.zombie_team)) {
        if (isalive(ai) && !is_true(ai.var_d45ca662) && !is_true(ai.marked_for_death)) {
            ...
            if (var_82ea43f2 || ai.var_6f84b820 !== #"normal") {
                ...
                ai.allowdeath.ai = 1;
                ai.no_powerups.ai = 1;
                ai.deathpoints_already_given.ai = 1;
                ai.marked_for_death.ai = 1;
    ...
    if (level.active_powerups.size < 75) {
        ...
        if (isdefined(e_powerup)) {
            ...
            if (isdefined(level.var_6463d67c)) {
                e_powerup.var_258c5fbc.e_powerup = level.var_6463d67c;
            }
        }
    }
    return e_powerup;
...
function private function_e1a11b1(var_8861fa85, n_radius, trigger, var_ab426dbb = 0) {
    ...
    n_radius_sq = function_a3f6cdac(n_radius);
    ...
    while (1) {
        foreach (n_obj_id in self.var_45f78aa4) {
            ...
            foreach (player in getplayers()) {
                ...
                } else if (distancesquared(player.origin, self.origin) < n_radius_sq) {
                    objective_setplayerusing(n_obj_id, player);
                    if (var_ab426dbb && var_13dc7b2a) {
                        objective_setinvisibletoplayer(n_obj_id, player);
                    }
...
function function_5d356609(aat_name, n_tier) {
    if (isplayer(self) && isdefined(aat_name)) {
        var_f35f3f4b = {#var_dacb4b0b:function_e3025ca5(), #var_c075f57d:get_round_number(), #var_9f39dfd4:function_f8d53445(), #var_93230b5d:n_tier, #var_27ad1f0f:hash(aat_name)};
                ...
                if (isdefined(machine)) {
                    ...
                    if (var_30300360 && !is_true(machine.var_c02c4d66)) {
                        if (!isdefined(player.var_4a501715[var_4e157e21])) {
                            ...
                            var_b0b08518.var_86e2d95e.var_b0b08518 = machine.model;
                        }
...
function private function_7757350c(player, machine) {
    var_2cdb84bb = machine.origin + machine function_2c662f72();
...
function function_ebd87099(player) {
    if (isdefined(level.var_23fc2144) && isdefined(level.var_f7794bb5)) {
        ...
        foreach (param in var_2a4e3502) {
            if (player.origin[2] >= param.origin[2] && player.origin[2] <= param.origin[2] + param.height && distance2dsquared(player.origin, param.origin) < function_a3f6cdac(param.radius)) {
                return param;
            }
        }
    }
...
function function_89dbd679(origin, radius, height, var_2d8730f7, var_b9e9cdf3) {
    ...
    level.var_23fc2144[level.var_23fc2144.size] = {#var_b9e9cdf3:var_b9e9cdf3, #var_2d8730f7:var_2d8730f7, #height:height, #radius:radius, #origin:origin};
    max_radius = 0;
        ...
        foreach (player in function_a1ef346b()) {
            ...
            if (!var_99442276 && player istouching(self)) {
                if (self function_4f593819(player)) {
                    ...
                    player.var_e62ad24a.player = self;
                } else {
            ...
            } else if (player.var_e62ad24a === self) {
                player globallogic::function_d96c031e();
                player.var_e62ad24a = undefined;
            }
        }
...
// Size: 0x290
function function_232a7f41(var_90de5) {
    var_90de5 playrumbleonentity(#"sr_transmitter_clear");
    a_vehicles = [];
    var_a8b5d9cc = arraycombine(getentitiesinradius(var_90de5.origin, 300, 12), getentitiesinradius(var_90de5.origin, 300, 14));
    foreach (vehicle in var_a8b5d9cc) {
        if (vehicle istouching(var_90de5)) {
        ...
        } else if (isdefined(var_90de5.trigger) && vehicle istouching(var_90de5.trigger)) {
            if (!isdefined(a_vehicles)) {
                a_vehicles = [];
            } else if (!isarray(a_vehicles)) {
                a_vehicles = array(a_vehicles);
    ...
    foreach (vehicle in a_vehicles) {
        if (!is_true(vehicle.var_2e436083)) {
            vehicle.var_2e436083.vehicle = 1;
...
function function_78e620d() {
    if (isvehicle(self)) {
        ...
        self namespace_43f477a6::function_fe45d0ae();
        wait(0.2);
    }
    if (isdefined(self)) {
...
function function_b42da08a(dynent) {
    ...
    if (isdefined(level.var_4b8c034d[dynent.var_15d44120])) {
        dynent.bundle.dynent = level.var_4b8c034d[dynent.var_15d44120];
    } else {
